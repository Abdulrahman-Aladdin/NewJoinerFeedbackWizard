...
SONAR_PROJECT_KEY = os.environ.get('SONAR_PROJECT_KEY')  # optional
...
def gh_search_issue_by_sonar_key(sonar_key):
    # use GitHub Search API to find if issue body contains sonar key
    q = f'repo:{owner}/{repo} in:body "sonar-issue-key:{sonar_key}"'
...
def list_github_sonar_issues(state='open', per_page=100):
    # list issues with label 'sonar' - pagination
    url = f"https://api.github.com/repos/{owner}/{repo}/issues"
    headers = {'Authorization': f'token {GITHUB_TOKEN}', 'Accept': 'application/vnd.github.v3+json'}
    page = 1
    results = []
...
def main():
    ...
    while True:
        ...
        for it in issues:
            ...
            # avoid duplicates
            if gh_search_issue_by_sonar_key(key):
                print("Already exists on GH:", key)
                continue
